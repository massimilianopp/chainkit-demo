...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (!chapterId) {
      res.setHeader("Cache-Control", "no-store");
      return res.status(400).json({ ok:false, error:"Missing chapterId" });
    }

    // 1) tente de réutiliser un PENDING existant (idempotent)
    const up = await sql`
      UPDATE purchases
   ...
   RETURNING id
    ...
    if (!reference) {
      ...
      const ins = await sql`
        ...
        VALUES (${proposed}::uuid, ${wallet}, ${chapterId}, ${CHAPTER_PRICE_RAW},
                ${TOMATO_MINT.toBase58()}, ${MERCHANT_WALLET.toBase58()}, 'PENDING')
      RETURNING id
      `;
      reference = ins?.[0]?.id;
    }
